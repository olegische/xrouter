...
func UpdateTaskByPlatform(platform constant.TaskPlatform, taskChannelM map[int][]string, taskM map[string]*model.Task) {
	...
	default:
		common.SysLog("Unknown platform")
	}
}
...
func UpdateSunoTaskAll(ctx context.Context, taskChannelM map[int][]string, taskM map[string]*model.Task) error {
	for channelId, taskIds := range taskChannelM {
		...
		if err != nil {
			common.LogError(ctx, fmt.Sprintf("Channel #%d async task update failed: %d", channelId, err.Error()))
		}
	}
	return nil
}
...
func updateSunoTaskAll(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	if !responseItems.IsSuccess() {
		common.SysLog(fmt.Sprintf("Channel #%d has incomplete tasks: %d, successfully retrieved task count: %d", channelId, len(taskIds), string(responseBody)))
		return err
	}
		...
		if responseItem.FailReason != "" || task.Status == model.TaskStatusFailure {
			...
			if err != nil {
				...
				if quota != 0 {
					...
					logContent := fmt.Sprintf("Async task execution failed %s, compensated %s", task.TaskID, common.LogQuota(quota))
