...
func getOidcUserInfoByCode(code string) (*OidcUser, error) {
	...
	if oidcResponse.AccessToken == "" {
		common.SysError("OIDC failed to get token, please check settings!")
		return nil, errors.New("OIDC failed to get token, please check settings!")
	}
	...
	if res2.StatusCode != http.StatusOK {
		common.SysError("OIDC failed to get user information! Please check settings!")
		return nil, errors.New("OIDC failed to get user information! Please check settings!")
	}
	...
	if oidcUser.OpenID == "" || oidcUser.Email == "" {
		common.SysError("OIDC user information is empty! Please check settings!")
...
func OidcAuth(c *gin.Context) {
	...
	if !system_setting.GetOIDCSettings().Enabled {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "Administrator has not enabled login and registration through OIDC",
		})
		return
	}
	code := c.Query("code")
