"""OpenRouter request and response mappers."""
import json
import uuid
...
from ..models.base.request import UsageRequest
from router.chat_completion.models.llm_gateway.request import LLMGatewayRequest


class OpenRouterMapper(BaseMapper):
    """Mapper for OpenRouter requests and responses."""
    ...
    def map_to_provider_request(self, request: ProviderRequest) -> Dict[str, Any]:
        """Map provider request to OpenRouter API request.
        ...
        try:
            if not isinstance(request, ProviderRequest):
                raise ProviderError(
                    code=400,
                    message="Failed to map request for OpenRouter",
                    details={"error": "Invalid request type"},
                )

            self.logger.debug(
                "Mapping provider request to OpenRouter API format",
                extra={
                    "model": request.model,
                    "temperature": request.temperature,
                    "max_tokens": request.max_tokens,
                    "request_id": request.request_id,
                    "has_reasoning": bool(request.reasoning),
                },
            )

            # Create OpenRouter request using LLMGatewayRequest model
            # Always set usage.include=true for OpenRouter requests
            # to get full usage details
            usage_request = UsageRequest(include=True)

            openrouter_request = LLMGatewayRequest(
                model=request.model,
                ...
                usage=usage_request.model_dump(),
                reasoning=request.reasoning.model_dump() if request.reasoning else None,
            )
            return cast(
                Dict[str, Any], openrouter_request.model_dump(exclude_none=True)
            )

        except Exception as e:
            error_request_id = str(uuid.uuid4())
            self.logger.error(
                "Failed to map request to OpenRouter API format",
                extra={
                    "error": str(e),
    ...
    def map_provider_stream_chunk(
        ...
        self.logger.debug(
            "Mapping OpenRouter API stream chunk",
            extra={
                "model": model,
        ...
        try:
            ...
            self.logger.debug(
                "Successfully mapped OpenRouter API stream chunk",
                extra={
            ...
            self.logger.error(
                "Failed to map OpenRouter API stream chunk",
                extra={
                    "error": str(e),
                    "request_id": request_id,
                },
            ...
            raise ProviderError(
                code=500,
                message="Failed to map OpenRouter API stream chunk",
                details={"error": str(e)},
            )
