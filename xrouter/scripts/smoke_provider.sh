...
if [[ $# -lt 1 ]]; then
  ...
  echo "providers: openrouter deepseek gigachat yandex ollama zai xrouter" >&2
  exit 1
fi
...
echo "[smoke][$provider] model=$model host=$HOST port=$PORT"
...
echo "[smoke][$provider] chat completions ok"
...
responses_text="$(echo "$responses_resp" | jq -r '.output_text // empty')"
if [[ -z "$responses_text" ]]; then
  echo "responses missing output_text" >&2
  echo "$responses_resp" >&2
  exit 1
fi

echo "[smoke][$provider] responses ok"
...
echo "[smoke][$provider] tool-like chat flow ok"
echo "[smoke][$provider] PASS"
