...
impl ProviderClient for DeepSeekClient {
    async fn generate(
        ...
        let (payload, normalization) =
            build_deepseek_payload(model, input, reasoning, tools, tool_choice);
        info!(
...
#[derive(Debug, Clone)]
pub(crate) struct DeepseekNormalization {
...
mod tests {
    ...
    fn keeps_only_function_tools_and_tracks_drops() {
        ...
        let normalized = normalize_tools_for_chat_completions(Some(&tools));
        assert_eq!(normalized.tools.len(), 1);
        assert_eq!(normalized.dropped_count, 2);
        assert_eq!(
            normalized.dropped_tool_types,
            vec!["web_search".to_string(), "local_shell".to_string()]
        );
    }

    #[test]
    fn maps_shorthand_function_tool_to_chat_shape() {
        let tools = vec![json!({
            "type": "function",
            "name": "exec_command",
        ...
        assert_eq!(normalized.tools[0]["function"]["name"], "exec_command");
        assert_eq!(normalized.tools[0]["function"]["description"], "run command");
    }
    ...
    fn normalizes_tool_choice_variants_for_chat_completions() {
        ...
        assert_eq!(
            normalize_tool_choice_for_chat_completions(Some(&json!({"type":"none"})), true),
            Some(json!("none"))
        );
